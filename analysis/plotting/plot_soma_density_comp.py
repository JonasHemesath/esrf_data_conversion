...
def plot_soma_density_per_brain_region_non_neurons_adjusted(data_per_brain_region, density_burek_path, output_dir, dark_mode=False, left_color='skyblue', right_color='salmon', comp_color='lightgray', tick_fontsize=10, title_fontsize=12, legend_fontsize=16):
    ...
    rects4 = ax.bar(x + width, density_burek_non_neurons, width, bottom=density_burek, label='Non-Neurons estimated (Olkowicz et al.)', color=comp_color, alpha=0.7)
...
def main():
    ...
    for brain_region_name, hemispheres in data_per_brain_region.items():
        for hemisphere, data in hemispheres.items():
            print(f"{brain_region_name} - {hemisphere.upper()} Hemisphere: Soma Count = {data['soma_count']}, Brain Region Volume = {data['brain_region_volume']:.2f} um³")
