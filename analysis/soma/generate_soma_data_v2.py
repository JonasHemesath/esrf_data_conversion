...
def _compute_soma_row_for_label(label):
    ...
    try:
        ...
        return (int(label), int(brain_region), surface_area, volume, convex_hull_volume)
...
class SomaDataGenerator:
    ...
    def iter_rows_parallel(self, num_workers=None, chunksize=10, show_progress=True):
        """
        Yields (label, brain_region, surface_area, volume, convex_hull_volume) in parallel.
        """
    ...
    def iter_rows_serial(self, show_progress=True):
        """
        Yields (label, brain_region, surface_area, volume, convex_hull_volume) serially.
        """
    ...
    def save_soma_data(self, output_file_csv, output_file_np=None, flush_every=200000):
        ...
        The .npy array has shape (max_label+1, 5) and columns:
          [label, brain_region, surface_area, volume, convex_hull_volume]
        ...
        with open(output_file_csv, "w") as f:
            ...
            for (label, brain_region, surface_area, volume, convex_hull_volume) in row_iter:
                # Write to memmap
                if mm is not None:
                    mm[label, :] = (label, brain_region, surface_area, volume, convex_hull_volume)

                # Write to CSV
                f.write(f"{label},{brain_region},{surface_area},{volume},{convex_hull_volume}\n")
                written += 1
