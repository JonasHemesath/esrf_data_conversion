...
def main():
    ...
    parser.add_argument("--region", type=int, help="Brain region ID for processing")
    
    args = parser.parse_args()

    # Load existing soma data
    soma_data = np.load(args.input_file)
    filtered_soma_data = soma_data[soma_data[:, 2] == args.region]  # Assuming brain region ID is in the third column
    

    # Add closest somata information
    updated_soma_data = add_closest_somata_info(filtered_soma_data)

    # Save the updated soma data
    np.save(args.output_file, updated_soma_data)
    print(f"Finished processing brain region {args.region} and saved to {args.output_file}")
