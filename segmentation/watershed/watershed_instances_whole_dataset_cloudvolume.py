            ...
            for x in range(x_i, x_chunks, args.step):
                ...
                for y in range(y_i, y_chunks, args.step):
                    ...
                    for z in range(z_i, z_chunks, args.step):
                        ...
                        processes.append([subprocess.Popen(['srun', '--time=7-0', '--gres=gpu:0', '--mem=400000', '--tasks', '1', '--cpus-per-task', '32', '--nice', 'python', '/cajal/nvmescratch/users/johem/esrf_data_conversion/segmentation/watershed/watershed_instances_block_cloudvolume.py',
