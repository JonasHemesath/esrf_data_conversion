    ...
    if args.mode == "train":
        ...
        for root, _, files in os.walk(args.train_data_dir):
            for file in files:
                ...
                myelin_path = os.path.join(root, f"{base_name}_Soma.tif")

                if not (os.path.exists(vessels_path) and os.path.exists(myelin_path)):
                    print(f"Missing label files for base '{base_name}': expected '{base_name}_BV.tif' and '{base_name}_Soma.tif'. Skipping.")
                    continue
        ...
        tifffile.imwrite(os.path.join(args.output_dir, "soma_prediction.tif"), myelin_pred)

        print(f"Semantic predictions for vessels/myelin saved to: {args.output_dir}")
