...
def extract_patches_trilinear_batch(
    img_1dhw: torch.Tensor,  # (1, D, H, W)
    starts_f_zyx: List[Tuple[float, float, float]],  # len=B
    roi_size_zyx: Tuple[int, int, int],
    padding_mode: str = "border",
) -> torch.Tensor:
    """
    Efficient batched trilinear patch extraction from one volume.
    Returns (B, 1, rz, ry, rx) tensor, B = number of starts.
    """
    rz, ry, rx = roi_size_zyx
    _, D, H, W = img_1dhw.shape
    device = img_1dhw.device
    ...
    # Prepare coordinate aranges once
    z_arange = torch.arange(rz, device=device, dtype=dtype)
    y_arange = torch.arange(ry, device=device, dtype=dtype)
    x_arange = torch.arange(rx, device=device, dtype=dtype)

    for z0, y0, x0 in starts_f_zyx:
        z = z_arange + z0
        y = y_arange + y0
        x = x_arange + x0
        ...
        zz, yy, xx = torch.meshgrid(z_norm, y_norm, x_norm, indexing="ij")
        grid = torch.stack((xx, yy, zz), dim=-1)  # (rz, ry, rx, 3)
        grids.append(grid)

    grid_batch = torch.stack(grids, dim=0)        # (B, rz, ry, rx, 3)
    img_batch = img_1dhw.expand(B, -1, -1, -1, -1) # (B, 1, D, H, W)
...
def multires_sliding_window_inference(
    ...
    padding_mode: str = "border",
    out_channels: int = None,   # <- add here!
) -> torch.Tensor:
    """
    ...
    def extract_patch_batch(batch_slices):
        """
        Returns a (B, C, roiD, roiH, roiW) batch of multi-res patches.
        """
        B = len(batch_slices)
        shape_ref = (D, H, W)

        # For each key (scale), we build a (B, 1, roiD, roiH, roiW) patch batch, then cat at channel dim
        patch_batches_per_scale = []
        ...
        for k in keys_order:
            ...
            if k == ref_key:
                # integer crops: get zyx starts
                z0 = torch.tensor([int(slc[0].start) for slc in batch_slices], device=v.device)
                y0 = torch.tensor([int(slc[1].start) for slc in batch_slices], device=v.device)
                x0 = torch.tensor([int(slc[2].start) for slc in batch_slices], device=v.device)
                # (B,)
                patches = []
                for b in range(B):
                    patches.append(
                        v[:, 
                        z0[b]:z0[b] + roi_size[0], 
                        y0[b]:y0[b] + roi_size[1], 
                        x0[b]:x0[b] + roi_size[2]
                        ]
                    )  # (1,roiD,roiH,roiW)
                patch_batches_per_scale.append(torch.stack(patches, dim=0))
            else:
                # Compute mapped starts
                mapped_starts = []
                for slc in batch_slices:
                    ...
                    mapped_starts.append(sf)
                batch_patch = extract_patches_trilinear_batch(
                    v, mapped_starts, roi_size, padding_mode=padding_mode
                ).squeeze(1)  # remove singleton channel (B,roiD,roiH,roiW)
                patch_batches_per_scale.append(batch_patch.unsqueeze(1))  # (B,1,roiD,roiH,roiW)

        # Concatenate along channel dimension
        patch_batch = torch.cat(patch_batches_per_scale, dim=1)  # (B,C,roiD,roiH,roiW)
        return patch_batch
...
def main(args):
    ...
    logits = multires_sliding_window_inference(
        ...
        ref_key="image_s0",
        keys_order=image_keys,
        padding_mode="border",
        out_channels=out_channels,   # <-- Pass this!
    )
