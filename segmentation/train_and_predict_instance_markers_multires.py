    ...
    if args.mode == "train":
        ...
        for root, _, files in os.walk(args.train_data_dir):
            for file in files:
                ...
                somata = tifffile.imread(soma_path).astype(np.uint8)

                # --- Create Soma Marker Map ---
                soma_instances, num_somas = label(somata > 0)
                if num_somas > 0:
                    centroids = center_of_mass(somata > 0, soma_instances, range(1, num_somas + 1))
                    marker_map = np.zeros(somata.shape, dtype=np.float32)
                    coords = np.array(centroids, dtype=int).T
                    # Ensure coords are within bounds
                    coords = np.clip(coords, 0, np.array(somata.shape)[:, np.newaxis] - 1)
                    marker_map[coords[0], coords[1], coords[2]] = 1.0
                    marker_map = gaussian_filter(marker_map, sigma=args.marker_creation_sigma)
                    if marker_map.max() > 0:
                        marker_map /= marker_map.max()
                else:
                    marker_map = np.zeros(somata.shape, dtype=np.float32)
        ...
        if not train_files:
            raise FileNotFoundError(
                f"No training sets found in {args.train_data_dir}. Expected *_raw.tif with matching *_Soma.tif."
            )
