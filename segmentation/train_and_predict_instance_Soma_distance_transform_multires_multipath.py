    ...
    if args.mode == "train":
        ...
        for root, _, files in os.walk(args.train_data_dir):
            for file in files:
                ...
                if not os.path.exists(soma_path) or not os.path.exists(marker_path):
                    continue
                ...
                item = {"label": combined_label_path, "marker": marker_path}
        ...
        spatial_keys = list(image_keys) + ["label", "marker"]
        ...
        loss_function_semantic = DiceLoss(to_onehot_y=True, softmax=True)
        loss_function_marker = MSELoss()
        ...
        for epoch in range(args.epochs):
            epoch_loss, epoch_loss_s, epoch_loss_m = 0, 0, 0
            for i, batch_data in enumerate(train_loader):
                ...
                markers = batch_data["marker"].to(device)           # (B,1,96,96,96)

                optimizer.zero_grad(set_to_none=True)
                outputs = model(inputs)
                outputs_semantic = outputs[:, :2, ...]  # first 2 channels for semantic
                outputs_marker = outputs[:, 2:, ...]    # last channel for marker regression
                loss_s = loss_function_semantic(outputs_semantic, labels)
                loss_m = loss_function_marker(outputs_marker, markers)
                loss = loss_s + loss_m * args.marker_loss_weight
                loss.backward()
                optimizer.step()

                epoch_loss += float(loss.item())
                epoch_loss_s += float(loss_s.item())
                epoch_loss_m += float(loss_m.item())
                print(f"E {epoch+1}/{args.epochs}, B {i+1}/{len(train_loader)}, Total Loss: {loss.item():.4f} (Sem: {loss_s.item():.4f}, Mark: {loss_m.item():.4f})")

            scheduler.step()
            avg_epoch_loss = epoch_loss / max(1, len(train_loader))
            epoch_losses.append(avg_epoch_loss)
            print(f"Epoch {epoch+1} avg loss: {avg_epoch_loss:.4f} (Sem: {epoch_loss_s/len(train_loader):.4f}, Mark: {epoch_loss_m/len(train_loader):.4f})")
        ...
        distance = torch.sigmoid(logits[:, 2]).squeeze(0).cpu().numpy()

        os.makedirs(args.output_dir, exist_ok=True)
        tifffile.imwrite(os.path.join(args.output_dir, "soma_prediction.tif"), soma_pred)
        tifffile.imwrite(os.path.join(args.output_dir, "marker_prediction.tif"), distance.astype(np.float32))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--marker_loss_weight', type=float, default=0.2, help='Weight for the marker prediction loss.')
