    ...
    if args.mode == "train":
        ...
        for root, _, files in os.walk(args.train_data_dir):
            for file in files:
                ...
                soma_path = os.path.join(root, f"{base_name}_marker.tif")
        ...
        for epoch in range(args.epochs):
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                labels = batch_data["label"].to(device)   # (B,1,96,96,96), long for one-hot

                optimizer.zero_grad(set_to_none=True)
                outputs = model(inputs)
                loss = loss_function(outputs, labels) * args.marker_loss_weight
                loss.backward()
                optimizer.step()
