...
def main(args):
    ...
    if args.mode == 'myelin_BV':
        ...
        out_vol = CloudVolume(args.output_name + '_BV', info=volume_info, bounded=False, progress=True, non_aligned_writes=False, parallel=1)
        ...
        out_vol = CloudVolume(args.output_name + '_myelin', info=volume_info, bounded=False, progress=True, non_aligned_writes=False, parallel=1)
        out_vol.provenance.description = f"Semantic segmentation ({args.mode}) generated by Pi2/Monai UNet model"
        out_vol.provenance.owners = ["jonas.hemesath@bi.mpg.de"]
        out_vol.commit_info()
        out_vol.commit_provenance()
        ...
        out_vol = CloudVolume(args.output_name + '_soma', info=volume_info, bounded=False, progress=True, non_aligned_writes=False, parallel=1)
        out_vol.provenance.description = f"Semantic segmentation (soma) generated by Pi2/Monai UNet model"
        out_vol.provenance.owners = ["jonas.hemesath@bi.mpg.de"]
        out_vol.commit_info()
        out_vol.commit_provenance()
        ...
        #out_vol = CloudVolume(args.output_name + '_dt', info=volume_info, bounded=False, progress=True, non_aligned_writes=False, parallel=1)
        #out_vol.provenance.description = f"Distance transform prediction generated by Pi2/Monai UNet model"
        #out_vol.provenance.owners = ["jonas.hemesath@bi.mpg.de"]
        #out_vol.commit_info()
        #out_vol.commit_provenance()
        z = zarr.create(
            ...
            store=args.output_name + '_dt.zarr',
            overwrite=True
        )
                ...
                for x in range(x_i, x_chunks, args.step):
                    ...
                    for y in range(y_i, y_chunks, args.step):
                        ...
                        for z in range(z_i, z_chunks, args.step):
                            ...
                            if args.export_debug:
                                processes.append(subprocess.Popen(['srun', '--time=7-0', '--gres=gpu:a40:1', '--mem=400000', '--tasks', '1', '--cpus-per-task', '32', '--nice', 'python', '/cajal/nvmescratch/users/johem/esrf_data_conversion/segmentation/inference/infer_block_semantic_multires.py',
