...
parser.add_argument('--mode', type=str, choices=['myelin', 'soma', 'BV'], required=True, 
                        help='Mode of the segmentation')
parser.add_argument('--block_shape', nargs=3, type=int, default=[1024, 1024, 1024], 
...
def main(args):
    ...
    stride = [s for s in args.block_shape]
    ...
    if stride == [1024, 1024, 1024]:
        aligned_writes = True
    else:
        aligned_writes = False
                ...
                for x in range(x_i, x_chunks, args.step):
                    ...
                    for y in range(y_i, y_chunks, args.step):
                        ...
                        for z in range(z_i, z_chunks, args.step):
                            ...
                            if args.export_debug:
                                processes.append(subprocess.Popen(['srun', '--time=7-0', '--gres=gpu:0', '--mem=60000', '--tasks', '1', '--cpus-per-task', '2', 'python', '/cajal/nvmescratch/users/johem/esrf_data_conversion/segmentation/masking/apply_mask_block_cloudvolume.py',
                                                                ...
                                                                '--mask_mip', str(args.mask_mip),
                                                                '--aligned_writes', str(aligned_writes),
