...
if os.path.isfile('percentiles.json'):
    with open('percentiles.json', 'r') as f:
            ...
            if file + '_' + str(i) not in percentiles[2]:
                try:
                    ...
                    percentiles[0].append(
                                np.percentile(im[mask==1], 0.0016)
                            )
                    percentiles[1].append(
                                np.percentile(im[mask==1], 99.9984)
                            )
                    percentiles[2].append(
                                file + '_' + str(i)
                            )
                    with open('percentiles.json', 'w') as json_file:
...
for file in os.listdir():
    if file[-4:] == 'tiff':
        ...
        for i in tqdm(range(z)):
            ...
            im[i,:,:][mask==1] = np.interp(im[i,:,:][mask==1], [min_perc, max_perc], [500, 65035])  # map all values in the circle to the new value range
        print('mapping finished')

        im_new = im.astype(np.uint16)
        ...
        ij.io().save(im_ij, file[0:-5] + '_16bit.tiff')
